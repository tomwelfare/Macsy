...
class TestBlackboardRetrieval(unittest.TestCase):
	...
	def setUp(self):
		...
		self.api = BlackboardAPI(self.settings, MongoClient=TestBlackboardRetrieval.client_constructor)
	...
	def test_bb_get_date(self):
		date = self.bb.get_date({'_id': ObjectId.from_datetime(dtparser.parse('21-10-2017'))})
		assert str(date) == '2017-10-21 00:00:00+00:00', 'bb.get_date(doc) found the wrong date: {}'.format(date)

if __name__ == '__main__':
    suite = unittest.defaultTestLoader.loadTestsFromTestCase(TestBlackboardRetrieval)
