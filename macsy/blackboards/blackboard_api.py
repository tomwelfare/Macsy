...
def validate_blackboard_name(fn):
...
class BlackboardAPI():
    ...
    def __init__(self, settings, MongoClient=MongoClient):
        self.__username = urllib.parse.quote_plus(settings[BlackboardAPI._setting_fields.get('username')])
        self.__password = urllib.parse.quote_plus(settings[BlackboardAPI._setting_fields.get('password')])
        self.__dbname = settings[BlackboardAPI._setting_fields.get('dbname')]
        self.__dburl = settings[BlackboardAPI._setting_fields.get('dburl')].replace('mongodb://','').strip('/')
    ...
    def _check_admin_attempt(self, settings):
        ...
        self.__password += str(BlackboardAPI._salt) if self.__username is not BlackboardAPI._admin_user else ''
